...
STRATEGY_TEMPLATES = {
    ...
    for obj in objects:
        if "{target_object}" in obj.name:
            ...
            break
    if target is None:
        target = find_object(name_contains="{target_object}")
    if not target:
        return {{"status": "failed", "reason": "未找到目标物体: {target_object}"}}
...
''',
    ...
    for obj in objects:
        if "{target_object}" in obj.name:
            top_obj = obj
        if "{reference_object}" in obj.name:
            bottom_obj = obj
    if not top_obj:
        top_obj = find_object(name_contains="{target_object}")
    if not bottom_obj:
        bottom_obj = find_object(name_contains="{reference_object}")
...
''',
    ...
    move_home(robot)
    return {{"status": "success", "sorted_count": len(targets)}}
''',
    ...
    for obj in objects:
        if obj.color and any(attr in obj.color.lower() for attr in {attributes_list}):
            matching.append(obj)
        elif any(name in obj.name for name in {target_objects_list}):
            matching.append(obj)
...
class StrategyGenerator:
    ...
    def __init__(
        ...
        self.llm_model = llm_model or os.environ.get("LLM_MODEL", "gpt-4o")
        self.system_prompt = self._load_system_prompt()

    def _load_system_prompt(self) -> str:
        """加载 CodeArts 策略生成 System Prompt"""
        prompt_path = Path(__file__).parent.parent.parent / "prompts" / "codearts_system_prompt.md"
    ...
    def _fill_template(self, action: str, template: str, intent: Dict[str, Any]) -> str:
        ...
        reference_object = intent.get("reference_object", "")
        ...
        if action == "sort_by_size":
            return template.format(
                target_objects_list=repr(target_objects if target_objects else [target_object]),
            )
