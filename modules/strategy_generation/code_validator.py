...
class CodeValidator:
    ...
    FORBIDDEN_ATTRIBUTES = {
        ...
        "check_output", "remove", "unlink", "rmtree",
    }

    # 只允许提示词约定的运行时模块，避免“未列入黑名单的任意 import”。
    ALLOWED_IMPORT_ROOTS = {"numpy", "isaac"}
    SAFE_METHOD_CALLS = {
        "get", "lower", "upper", "strip", "startswith", "endswith",
        "append", "extend", "sort", "copy", "keys", "values",
    }
    ...
    def validate_security(cls, code: str) -> Tuple[bool, List[str]]:
        """安全检查 — 基于 AST 的导入、调用和危险属性校验。"""
        violations = []

        try:
            tree = ast.parse(code)
        except SyntaxError as exc:
            return False, [f"[BLOCK] 无法解析代码进行安全检查: {exc.msg}"]
        ...
        # 检查导入：黑名单模块明确阻断，其余模块也必须属于运行时白名单。
        for node in ast.walk(tree):
            ...
            for module in modules:
                ...
                if root in cls.FORBIDDEN_MODULES:
                    violations.append(f"[BLOCK] 禁止导入模块: `{root}`")
                elif root not in cls.ALLOWED_IMPORT_ROOTS:
                    violations.append(f"[BLOCK] 未允许导入模块: `{module}`")

        for node in ast.walk(tree):
            if not isinstance(node, ast.Call):
                continue

            chain = cls._attribute_chain(node.func)
            if not chain:
                continue

            if len(chain) == 1:
                func = chain[0]
                if func in cls.FORBIDDEN_FUNCTIONS:
                    violations.append(f"[BLOCK] 禁止调用函数: `{func}()`")
                elif (
                    func not in cls.ALLOWED_API_CALLS
                    and not func.startswith("task_")
                    and func not in defined_functions
                ):
                    violations.append(
                        f"[WARN] 未知函数调用: `{func}()` — 请确认在元 API 范围内"
                    )
                continue

            root, leaf = chain[0], chain[-1]
            call_name = ".".join(chain)
            is_allowed_api_call = any(
                call_name == allowed or call_name.startswith(allowed + ".")
                for allowed in cls.ALLOWED_API_CALLS
            )
            if root in cls.FORBIDDEN_MODULES or leaf in cls.FORBIDDEN_FUNCTIONS:
                violations.append(f"[BLOCK] 禁止调用函数: `{'.'.join(chain)}()`")
            elif leaf in cls.FORBIDDEN_ATTRIBUTES:
                violations.append(f"[BLOCK] 禁止调用属性: `{'.'.join(chain)}()`")
            elif is_allowed_api_call or root == "robot" or leaf in cls.SAFE_METHOD_CALLS:
                continue
            else:
                violations.append(
                    f"[WARN] 未知函数调用: `{'.'.join(chain)}()` — 请确认在元 API 范围内"
                )
    ...
    def _attribute_chain(node: ast.AST) -> List[str]:
        """把 obj.method 或 np.linalg.norm 展开为名称链。"""
        parts = []
        current = node
    ...
    def validate_gripper_force(cls, code: str) -> Tuple[bool, List[str]]:
        ...
        for match in re.finditer(r'close_gripper\s*\(\s*([^)]+)\)', code):
            ...
            try:
                ...
                if arg not in {"DEFAULT_FORCE", "SAFE_FORCE"}:
                    warnings.append(
                        f"[WARN] 无法静态确认 close_gripper({arg}) 的夹爪力范围"
                    )
    ...
    def validate_safety_assertions(cls, code: str) -> Tuple[bool, List[str]]:
        ...
        # 高层动作库负责内部运动安全；只有直接操作低层位姿时才要求显式高度断言。
        has_direct_pose_motion = bool(re.search(r'\bmove_to_pose\s*\(', code))
        has_z_guard = bool(
            re.search(r'assert\s+.*(?:z|Z)\s*>=\s*0\.02', code)
            or re.search(r'\b(?:SAFE_Z|PLACE_Z)\b', code)
        )
        if has_direct_pose_motion and not has_z_guard:
            warnings.append("[WARN] 直接位姿运动缺少 Z 轴安全高度保护")

        if re.search(r'\bclose_gripper\s*\(', code) and not re.search(
            r'\bverify_grasp\s*\(', code
        ):
            warnings.append("[WARN] close_gripper() 调用后应有 verify_grasp() 检查")

        # 对明显的低于安全高度的字面量直接阻断；变量形式交由运行时安全层继续检查。
        try:
            tree = ast.parse(code)
            for node in ast.walk(tree):
                if not isinstance(node, ast.Call):
                    continue
                chain = cls._attribute_chain(node.func)
                if not chain or chain[-1] != "move_to_pose" or len(node.args) < 3:
                    continue
                z_arg = node.args[2]
                if isinstance(z_arg, (ast.Constant, ast.Num)) and isinstance(z_arg.value, (int, float)):
                    if float(z_arg.value) < 0.02:
                        warnings.append("[WARN] move_to_pose 的 z 字面量低于安全高度 0.02m")
        except SyntaxError:
            warnings.append("[WARN] 无法解析运动代码进行物理安全检查")
    ...
    def full_validation(cls, code: str) -> dict:
        ...
        passed = syntax_ok and security_ok and safety_ok and entry_ok and force_ok
        ...
        return {
            "passed": passed,
            "stage": (
                "SECURITY" if not security_ok
                else "SAFETY" if not safety_ok
                else "ENTRY" if not entry_ok
                else "GRIPPER_FORCE" if not force_ok
                else "DONE"
            ),
            "syntax": (syntax_ok, syntax_msg),
            ...
            "summary": (
                ...
                if passed
                    ...
                    "[FAIL] 校验失败 — "
                    f"安全违规 {len(security_violations)}，"
                    f"物理警告 {len(all_warnings)}"
                )
            ),
        }
