...
$runId = "gt-final-" + (Get-Date -Format "yyyyMMdd-HHmmss")
...
function Invoke-SshChecked([string]$Command) {
    & ssh -n -T -p $Port -o ConnectTimeout=8 -o StrictHostKeyChecking=no $remoteSpec $Command
    if ($LASTEXITCODE -ne 0) { throw "远程命令失败，退出码 $LASTEXITCODE" }
}
function Copy-ToRemote([string]$LocalPath, [string]$RemotePath) {
    & scp -P $Port -o ConnectTimeout=8 -o StrictHostKeyChecking=no $LocalPath "$remoteSpec`:$RemotePath"
    if ($LASTEXITCODE -ne 0) { throw "上传失败，退出码 $LASTEXITCODE" }
}
function Copy-FromRemote([string]$RemotePath, [string]$LocalPath) {
    & scp -P $Port -o ConnectTimeout=8 -o StrictHostKeyChecking=no "$remoteSpec`:$RemotePath" $LocalPath
...
nohup setsid docker run --rm --entrypoint bash --gpus 'device=0' --network none -u 1234:1234 \
  ...
  -lc 'cd /isaac-sim && ./python.sh /workspace/tools/run_ground_truth_executor_acceptance_v4.py --device $Device --result-dir /workspace/results --strategy-file /workspace/live_chain_ab.json --/app/headless=true --/persistent/isaac/asset_root/default=/isaacsim_assets/Assets/Isaac/6.0' \
  > '$remoteRoot/results/container.log' 2>&1 < /dev/null &
docker_pid=`$!
for attempt in `$(seq 1 240); do if [ -s '$remoteRoot/results/perception.json' ] && [ -s '$remoteRoot/results/execution.json' ]; then echo REPORT_READY; exit 0; fi; if ! kill -0 "`$docker_pid" 2>/dev/null; then tail -n 80 '$remoteRoot/results/container.log' || true; exit 1; fi; sleep 2; done
...
finally { if (Test-Path -LiteralPath $bundle) { Remove-Item -LiteralPath $bundle -Force -ErrorAction SilentlyContinue } }
